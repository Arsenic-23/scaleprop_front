...
const mapErrorToMessage = (code: string): string => {
  ...
  if (lower.includes("too-many-requests")) return "Too many attempts. Try later.";
  if (lower.includes("network")) return "Network error. Check connection.";
...
const LoginForm: React.FC<LoginFormProps> = ({ onSuccess, onSwitchToRegister }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { exists, error } = await checkEmailExists(trimmedEmail);
    if (error) {
      setFormError("Error checking email.");
      setLoading(false);
      return;
    }
    if (!exists) {
    ...
    try {
      ...
      if (!user.emailVerified) {
        await signOut(auth);
        setFormError("Email not verified. Please check your inbox.");
        setLoading(false);
        return;
      }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        ...
        <div className="relative">
          <input
            ...
            className="w-full p-3 rounded-lg bg-black border border-white text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white pr-10"
          />
          <button
            type="button"
